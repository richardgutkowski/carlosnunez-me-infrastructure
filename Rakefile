...
task :print_help do
  ...
  puts "Deploys this infrastructure onto the cloud provider or hardware of your choice.\n\n"
  ...
  all_env_vars.each do |env_var, env_var_properties|
    ...
    puts "#{' '*3}#{env_var}:".yellow
    puts "#{' '*5}#{env_var_properties[:description]}"
    if not env_var_properties[:supported_values].nil?
      print "#{' '*5}Required values: ".cyan
...
namespace :prerequisites do
  ...
  task process_env_vars: :dotenv do
    @options = process_env_vars(required_env_vars:@REQUIRED_ENV_VARS,
...
go get github.com/palantir/tfjson 2>/dev/null; }; echo $?`
 ...
 grep #{@REQUIRED_BINARY_VERSIONS[:terraform_for_tfjson]}`
    if old_terraform_version.empty?
      puts "You don't have Terraform version #{@REQUIRED_BINARY_VERSIONS[:terraform_for_tfjson]} installed. \
